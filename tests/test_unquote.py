...
from protego import _hexescape, _unquote


def unquote(url, ignore='', errors='replace'):
    return _unquote(url, ignore, errors)


def hexescape(char):
    return _hexescape(char)
...
class TestUnquote(TestCase):
    ...
    def test_unquote_with_unicode(self):
        ...
        expect = u'br\u00fcckner_sapporo_20050930.doc'
        result = unquote(given)
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
        ...
        given = "%F3%B1"
        expect = u"\ufffd"                   # Replacement character
        result = unquote(given)
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))

        # Decode with UTF-8, invalid sequence, replace errors
        result = unquote(given, errors="replace")
        self.assertEqual(expect, result,
                         "using unquote(): %r != %r" % (expect, result))
